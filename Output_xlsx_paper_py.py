...
def formatDataForPaperOutput(corpus: dict[str,pd.DataFrame]):
    ages = bdf.getAvailableAges(corpus)
    ...
    for i in ages:
        if i<15:
            sub_corpora.append(bdf.cleanWordBeginnings(bdf.cleanLemmas(bdf.getDistinctSubCorp(corpus, i))))
        else:
            over_15.append(bdf.cleanWordBeginnings(bdf.cleanLemmas(bdf.getDistinctSubCorp(corpus, i))))
    ...
    for sub_corp in sub_corpora:
        ...
        #Add to dictionary
        ready_dfs[key] = filtered_data.sort_values('text')
    ...
    filtered_data['First Age Encountered'] = [word_age_appearances[x] for x in filtered_data['text'].to_numpy(dtype='str')]
    #Add to dictionary
    ready_dfs['Whole'] = filtered_data.sort_values('text')
...
def writePaperOutput(ready_dfs: dict[str:pd.DataFrame]):
    with pd.ExcelWriter("Data/FCBLex_data_output.xlsx") as writer:
        for df in ready_dfs:
            ready_dfs[df].to_excel(writer, sheet_name=df, index=False)
            print(df+" done!")

dfs = formatDataForPaperOutput(sentences)
